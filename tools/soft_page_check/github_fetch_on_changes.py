...
from paths import gh_release_fetch_root

HERE = Path(__file__).resolve().parent
REPORTS = HERE / "reports"
GH_ROOT = gh_release_fetch_root()
...
def _import_gh_release_fetch():
    """只读引用 gh-release-fetch/auto_update.py。"""
    auto_update_py = GH_ROOT / "auto_update.py"
    if not auto_update_py.is_file():
        raise FileNotFoundError(f"缺少 gh-release-fetch 目录或 auto_update.py: {GH_ROOT}")
    gh_path = str(GH_ROOT)
...
def main() -> int:
    ...
    if disabled:
        ...
        for app in disabled:
            print(f"  - {app['id']}")
        print("  请在 gh-release-fetch/apps/ 对应 JSON 里设 enabled=true 后重试")

...
echo   [6] 打开 HTML 报告页
echo   [7] list 三站检查（系统+移动 分开，hybase/dayanzai/appx64）
echo   [0] 退出
echo.
set /p "MENU=请选择 [0-7]: "
if "%MENU%"=="1" goto sop
...
echo.
echo  --- 正在运行 A 类快检 ---
call "%~dp0refresh_urls.bat" core nopause
...
call monthly_check_site.bat 423down
goto menu

:xiazai
echo.
echo  --- 7xiazai 软件详情页 title ---
call monthly_check_site.bat 7xiazai
goto menu

:list_sites
echo.
echo  --- list 三站（hybase + dayanzai + appx64）---
...
if not exist "%ROOT%\generate_and_push.bat" (
    echo.
    echo  [提示] 当前为 gh-release-fetch 仓库，无 generate_and_push.bat / software 发布流程。
    echo         Git 线更新请用仓库根目录: lookup_app.bat / run_saved_apps.bat
    pause
    goto menu
)
cd /d "%ROOT%"
...
if not exist "%ROOT%\generate_and_push.bat" (
    echo.
    echo  [提示] 当前为 gh-release-fetch 仓库，无 generate_and_push.bat。
    echo         若只更新 GitHub Releases 清单，请用 run_saved_apps.bat 等。
    pause
    goto menu
)

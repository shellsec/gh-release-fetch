...
def load_sync_dirs() -> list[str]:
    ...
    print(f"提示：未找到 config.json，A 类关键词仅来自 build_watchlist.py 内 A_PATTERNS")
    return list(A_PATTERNS.keys())
...
def build() -> dict:
    if not PAGES_FILE.exists():
        raise FileNotFoundError(f"缺少 {PAGES_FILE}，请先运行 refresh_urls.bat pages")

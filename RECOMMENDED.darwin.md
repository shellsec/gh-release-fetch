...
| `github_copilot_cli` | GitHub Copilot CLI（macOS Apple Silicon tar.gz） | `github/copilot-cli` |
| `goose_ai` | Goose（macOS Apple Silicon tar.gz） | `block/goose` |
| `goose_desktop` | Goose 桌面版 | `block/goose` |
| `gpt4all` | GPT4All（本地运行 LLM 的桌面客户端） | `nomic-ai/gpt4all` |
| `headroom` | Headroom（LLM 输入压缩库，macOS arm64 wheel） | `chopratejas/headroom` |
| `immersive_translate` | 沉浸式翻译（Chrome 扩展 zip；解压后开发者模式加载） | `immersive-translate/immersive-translate` |
| `jan` | Jan（离线优先的本地 AI 聊天客户端） | `janhq/jan` |
| `kilocode` | Kilo Code（开源 AI 编程 VS Code 扩展） | `Kilo-Org/kilocode` |
| `kiro` | Kiro CLI macOS（manifest） | `-` |
...
| id | Intro | repo |
|----|-------|------|
| `aria2` | aria2：自 release-1.36 起 GitHub Release 仅提供源码包与 Windows/Android 预编译；macOS 请用 br... | `aria2/aria2` |
...
| `gallery_dl` | gallery-dl（macOS 二进制） | `mikf/gallery-dl` |
...
| `lux` | lux（原 annie，命令行抓取流媒体/站点视频；Apple Silicon 用 arm64 包） | `iawia002/lux` |
...
| id | Intro | repo |
|----|-------|------|
| `bisq` | Bisq（去中心化加密货币 P2P 交易，macOS dmg） | `bisq-network/bisq` |
| `bitcoin_core` | Bitcoin Core | `bitcoin-core/gui` |
| `electrum` | Electrum（macOS dmg） | `spesmilo/electrum` |
| `feather_wallet` | Feather Wallet | `feather-wallet/feather` |
| `feather_wallet_mac_arm64` | Feather Wallet（Monero，Apple Silicon zip） | `feather-wallet/feather` |
| `feather_wallet_mac_intel` | Feather Wallet（Monero，Intel mac zip） | `feather-wallet/feather` |
| `lnd_darwin_amd64` | LND（Lightning Network，macOS amd64 发行包） | `lightningnetwork/lnd` |
| `lnd_darwin_arm64` | LND（Lightning Network，macOS arm64 发行包） | `lightningnetwork/lnd` |
| `octobot` | OctoBot（加密交易机器人，macOS arm64） | `Drakkar-Software/OctoBot` |
| `sparrow_wallet` | Sparrow Wallet（Bitcoin） | `sparrowwallet/sparrow` |
| `sparrow_wallet_arm64` | Sparrow Wallet（比特币钱包，Apple Silicon dmg） | `sparrowwallet/sparrow` |
| `sparrow_wallet_intel` | Sparrow Wallet（比特币钱包，Intel macOS dmg） | `sparrowwallet/sparrow` |
| `specter_desktop` | Specter Desktop（比特币多签/硬件钱包，macOS dmg） | `cryptoadvance/specter-desktop` |
...
| `oh_my_posh_darwin_intel` | Oh My Posh（macOS Intel 二进制） | `JanDeDobbeleer/oh-my-posh` |
| `procs` | - | `dalance/procs` |
...
| `vlc` | VLC 媒体播放器（macOS dmg） | `videolan/vlc` |
...
| `keepass` | KeePass（macOS 无官方版；请用 keepassxc） | `KeePass/KeePass` |
...
| `owasp_zap` | OWASP ZAP（macOS 包） | `zaproxy/zaproxy` |
...
| id | Intro | repo |
|----|-------|------|
| `act` | act（macOS arm64） | `nektos/act` |
...
| `go` | Go 语言（macOS pkg，go.dev/dl；非 GoLand） | `golang/go` |
...
| `ohmyshot_darwin_intel` | OhMyShot（macOS Intel dmg） | `redf0x1/ohmyshot-releases` |
| `openshots` | OpenShots（macOS Apple Silicon dmg） | `Tracekit-Dev/openshots` |
| `openshots_darwin_intel` | OpenShots（macOS Intel dmg） | `Tracekit-Dev/openshots` |
...
| `redis_insight` | Redis Insight（macOS arm64 dmg） | `RedisInsight/RedisInsight` |
| `sqlc` | sqlc | `sqlc-dev/sqlc` |
| `sqlitebrowser` | DB Browser for SQLite（macOS dmg） | `sqlitebrowser/sqlitebrowser` |
...
| `antigravity` | Antigravity dmg（manifest） | `-` |
...
| `pulsar` | Pulsar（Atom 继任，macOS zip） | `pulsar-edit/pulsar` |
| `qoder` | Qoder dmg（manifest） | `-` |
| `qoderwork` | QoderWork dmg（manifest） | `-` |
| `skylark` | - | `adonais/skylark` |
| `sublime_merge` | Sublime Merge | `sublimehq/sublime_merge` |
| `trae` | Trae macOS dmg（manifest） | `-` |
...
| `syncplay` | Syncplay（macOS dmg） | `Syncplay/syncplay` |
| `vidcutter` | VidCutter（macOS dmg） | `ozmartian/vidcutter` |
